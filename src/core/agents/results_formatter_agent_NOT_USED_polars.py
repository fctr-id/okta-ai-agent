...
try:
    ...
    coding_model = ModelConfig.get_model(ModelType.REASONING)
except ImportError:
...
def _create_sample_data(results: Dict[str, Any], max_samples: int = 2) -> Dict[str, Any]:
    """Create RAW sample data for LLM analysis - NO pre-processing"""
    samples = {}
    for step_name, step_data in results.items():
        if isinstance(step_data, list) and step_data:
            # Send RAW samples - let LLM see the actual structure
...
# Note: Flattening is now handled by Polars patterns, not Python preprocessing
