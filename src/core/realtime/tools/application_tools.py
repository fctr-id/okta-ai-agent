...
async def get_application_details(client, app_id):
    ...
    *   **`app_id`** (Required, String):
    ...
    app_result = await paginate_results(
        "get_application",
        method_args=[app_id],
    ...
    # Check for errors
    if isinstance(app_result, dict) and "status" in app_result and app_result["status"] == "error":
        return app_result
    
    # Check if application was found
    if not app_result or (isinstance(app_result, list) and len(app_result) == 0):
        return {"status": "not_found", "entity": "application", "id": app_id}
    
    # Handle case where response might be a list with one application
    if isinstance(app_result, list) and len(app_result) > 0:
        app = app_result[0]
    else:
        app = app_result
    
    # Return the application data directly - the results processor will handle formatting
    return app
    ```
    ...
    - Data returned by paginate_results is already in dictionary format (not objects)
    - Access fields using dictionary syntax: item["property"]["field"] (not object.attribute syntax)    
    - Use method_args=[app_id] to pass the application ID as a positional argument
    ...
    - The results processor will handle filtering fields based on query context
    - Policy IDs can be found in the _links section of the response
    """
    # Implementation will be handled by code generation
    # This function is just a placeholder for registration
    pass
