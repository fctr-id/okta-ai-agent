...
class ResultsProcessorAgent:
    ...
    def _create_sample_data_prompt(self, query: str, results: Dict[str, Any], plan: Any) -> str:
        ...
        ## Execution Steps generated by to implement the execution plan
        ##{execution_steps_text}
        ...
        Based on the execution steps, understand that:
        - Key "1" contains results from Step 1 (typically a list of users)
        - Key "2" contains results from Step 2 (typically the FILTERED users without push factors)
        - The data in key "2" is NOT factor data for each user in key "1" - it is the FINAL FILTERED result
        ...
        - Look at the keys in the sample data to understand what each step returns
        - Identify how the data from different steps relates to each other
        - Focus on creating a clear, user-friendly presentation of the data
