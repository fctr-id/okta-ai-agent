...
# Simple agent with essential PydanticAI features only
results_formatter = Agent(
    formatter_model,
    output_type=FormattedOutput,
...
async def format_results(query: str, results: Dict[str, Any]) -> Dict[str, Any]:
    """Simple function with basic token tracking"""
    
    prompt = f"""Query: {query}
Results: {json.dumps(results, indent=2, default=str)}
Please format these results professionally."""
    
    try:
        if config['enable_token_reporting']:
            logger.info("🎨 [Results Formatter Agent] Processing with enhanced PydanticAI...")
        
        # Run formatter
        result = await results_formatter.run(prompt)
        
        # Simple validation outside LLM
        if result.data and is_valid_format(result.data):
            logger.info("✅ [Results Formatter Agent] Successfully processed with validation")
        else:
            logger.warning("⚠️ [Results Formatter Agent] Format issues detected - but continuing")
