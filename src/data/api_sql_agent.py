...
def load_api_sql_agent_system_prompt() -> str:
    """Load system prompt from external file"""
    try:
        prompt_file = "api_sql_agent_system_prompt.txt"
        with open(prompt_file, 'r', encoding='utf-8') as f:
            return f.read().strip()
    except FileNotFoundError:
        logger.error(f"System prompt file not found: {prompt_file}")
        raise FileNotFoundError(f"Could not load API SQL agent system prompt from {prompt_file}")

class ApiSqlAgent:
    """Internal SQL agent for API data processing - NOT exposed to users"""
    
    def __init__(self):
        self.system_prompt = load_api_sql_agent_system_prompt()
        self.agent = Agent(
            model=model,
    ...
    def get_database_schema() -> str:
        ...
        from shared_schema import get_okta_database_schema
