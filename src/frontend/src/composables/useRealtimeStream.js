...
export function useRealtimeStream() {
    ...
    try {
      ...
      eventSource.onmessage = (event) => {
        ...
        try {
          ...
          if (data.plan) handlePlanStatusEvent({ data: JSON.stringify({ status: 'generated', plan: data.plan }) })
  ...
  const handlePlanStatusEvent = (event) => {
    try {
      ...
      if (data.status === 'generated' || data.status === 'starting_execution' || data.status === 'running_execution') {
        execution.planGenerated = true
        execution.status = 'executing'
      }
    } catch (err) {
